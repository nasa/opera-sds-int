...
for burst_id in "${burst_ids[@]}"; do
  ...
  burst_id_pattern="OPERA_L2_CSLC-S1_${burst_id_replace_underscores}_*"

  output_file=$(ls ${output_dir}/${burst_id_pattern}/*.h5)
  expected_file=$(ls ${expected_dir}/expected_output_data/${burst_id_pattern}/*.h5)

  python3 cslc_compare.py -r ${expected_file} -s ${output_file} -p CSLC
done
