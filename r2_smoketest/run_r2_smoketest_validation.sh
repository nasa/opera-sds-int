...
# Submit query job
python3 ~/mozart/ops/opera-pcm/data_subscriber/daac_data_subscriber.py query -c SENTINEL-1A_SLC --processing-mode=reprocessing --job-queue=opera-job_worker-slc_data_download --chunk-size=1 --native-id=S1A_IW_SLC__1SDV_20220501T015035_20220501T015102_043011_0522A4_42CC*

# Download and unzip gold files.
wget https://artifactory-fn.jpl.nasa.gov:443/artifactory/general-stage/gov/nasa/jpl/opera/sds/pcm/testdata_R2.0.0/deployment_smoke_test_cslc_s1_${cslc_v}.zip
wget https://artifactory-fn.jpl.nasa.gov:443/artifactory/general-stage/gov/nasa/jpl/opera/sds/pcm/testdata_R2.0.0/deployment_smoke_test_rtc_s1_${rtc_v}.zip
unzip deployment_smoke_test_rtc_s1_${rtc_v}.zip
unzip deployment_smoke_test_cslc_s1_${cslc_v}.zip
...
./rtc_compare.sh ./output_dir_rtc ./deployment_smoke_test_rtc_s1_${rtc_v} | grep "file: OPERA_L2_RTC-S1_T064"

echo "Comparing CSLC products against gold files"
./cslc_compare.sh ./output_dir_cslc ./deployment_smoke_test_cslc_s1_${cslc_v}